#!/usr/bin/env python3
"""Frozen legacy (pre-§4-O) hallucination analyzer.

This file is a snapshot of the v1 analyzer state used to regenerate the
§6-B writeup ablation table. It exists alongside the current
hallucination_analysis.py and is NOT used in normal v1.0 runs.

Differences vs the live analyzer (the §6-B variables under study):
  1. string_in_universe still has the v1 strict pass that admits 2-char
     alpha tokens (mc/wc/gl) as composed-string members. Legacy matcher.
  2. build_packet_universe section (3) live-parses every PDF / xlsx / csv
     in the packet via pdftotext + easyocr + openpyxl. Doc-parsing path.

Run this directly to reproduce §6-B row 1 (legacy matcher + docs). To
reproduce row 2 (tightened matcher + docs), cherry-pick the live
analyzer's tightened string_in_universe into this file.

System deps for this file: pdftotext (poppler-utils). Python deps:
easyocr, pdf2image, numpy, openpyxl.

Usage (legacy):
    python scripts/hallucination_analysis_legacy.py \\
        --ground-truth ground_truth/ \\
        --results results/ \\
        --output results/hallucination_report_legacy.json
"""
...
def build_packet_universe(gt_data: dict, gt_file: Path | None = None) -> tuple[set[str], list[float]]:
    ...
    LEGACY (pre-§4-O) — this snapshot keeps section (3) live PDF / xlsx /
    csv parsing via pdftotext + easyocr + openpyxl. The current analyzer
    drops that path and runs JSON-only; this file exists solely to
    reproduce §6-B row 1 (legacy matcher + parsed docs).
    """
    strings: set[str] = set()
    numbers: list[float] = []
